...
description: Slidecast 뷰어에 self-contained HTML 덱을 업로드/수정(버전)/삭제한다. 호출자의 AWS 자격증명으로 S3+DynamoDB에 직접 반영한다. "슬라이드 뷰어에 올려", "slidecast append", "덱 배포"에 트리거.
---
...
## 사전 요구
- AWS 자격증명 설정 (`export AWS_PROFILE=<your-profile>` 또는 기본 자격증명 체인) · `export AWS_REGION=us-east-1`
...
- 영구 삭제: `python3 slidecast_append.py --delete <deckId> --hard`
  - slides/, thumbnails/, pdfs/, public/{token}/ prefix와 alias/public 예약을 모두 정리한다.
- 공개 공유 켜기: `python3 slidecast_append.py --share <deckId>`
  - 현재 버전을 `public/{token}/index.html`로 복사하고 `/p/{token}` 경로를 출력한다. 이미 공유 중이면 같은 토큰을 반환한다.
- 공개 공유 끄기: `python3 slidecast_append.py --unshare <deckId>`
  - `public/{token}/` 오브젝트와 예약을 삭제하고 덱의 publicToken을 비운다. 재공유 시 새 토큰이 발급된다.
